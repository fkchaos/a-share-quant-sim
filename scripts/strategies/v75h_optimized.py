#!/usr/bin/env python3
"""v75h: 因子权重优化版（基于v75f广度过滤）
优化权重：突破0.35/放量0.30/流动性0.35
全量回测Sharpe 0.8033（+28.9% vs v75f默认权重）

基于v75系列因子权重扫描结果：
- 流动性权重从0.25提升到0.35（Sharpe提升最大）
- 突破权重从0.45降到0.35（减少过拟合）
- 放量权重保持0.30
"""
import numpy as np
import pandas as pd
...
def calc_factors_v75h(close_panel, volume_panel, amount_panel, high_panel, low_panel, open_panel, extra_data=None):
    weights = {
...
def select_stocks_v75h(factors, date, close_panel, volume_panel, amount_panel,
