#!/usr/bin/env python3
"""v61b + 情绪择时 近三年数据扫描（2024-2026）"""
import sys, os, json, time
sys.path.insert(0, '/root/a-share-quant-sim')
import sqlite3, numpy as np, pandas as pd

RESULT_FILE = '/root/a-share-quant-sim/scripts/tools/v61b_sentiment_3yr_results.json'
LOG_FILE = '/root/a-share-quant-sim/scripts/tools/v61b_sentiment_3yr.log'
...
def load_data():
    ...
    sql = f"""SELECT code, date, open, high, low, close, volume
              ...
              AND date >= '2023-06-01' AND date <= '2026-06-29'
              ORDER BY code, date"""
...
def run_fold(data, test_start, test_end, rebal, top_n, sl, tp, hold_max,
             sentiment_threshold, sentiment_window, cold_mode=True):
    close = data['close']
    turnover = data['turnover']
...
def main():
    ...
    data = load_data()
    # 近三年：从2024-01-01开始，但需要前面的数据计算因子
    dates = sorted(data['close'].index)
    start_idx = next(i for i, d in enumerate(dates) if d >= pd.Timestamp('2024-01-01'))
    ...
    total_configs = len(MODES) * len(THRESHOLDS) * len(WINDOWS)
    ...
    for mode_name, cold_mode in MODES:
        for thresh in THRESHOLDS:
            for window in WINDOWS:
                key = f"{mode_name}_{thresh}_{window}"
                
                if key in results:
                    r = results[key]
                    log(f"  [跳过] {mode_name} 阈值={thresh}, 窗口={window}: 夏普={r['sharpe']:+.3f}")
                    continue
                ...
                if fold_results:
                    ...
                    results[key] = {
                        'mode': mode_name, 'thresh': thresh, 'window': window,
                    ...
                    log(f"  [{mode_name}] 阈值={thresh}, 窗口={window}: 夏普={avg_sharpe:+.3f}, 收益={avg_ret:+.1f}%, 回撤={avg_dd:+.1f}%, 正fold={pos}/{nf} ({elapsed:.1f}s)")
    ...
    log(f"\n{'模式':<6} {'阈值':>6} {'窗口':>6} {'收益':>10} {'夏普':>8} {'回撤':>8} {'正fold':>8}")
    log("-" * 70)
    
    for r in sorted_results[:30]:
        marker = "✅" if r['sharpe'] > 1.0 and r['pos_rate'] >= 60 else "  "
        log(f"{r['mode']:<6} {r['thresh']:>6.1f} {r['window']:>6} {r['ret']:>+9.1f}% {r['sharpe']:>+7.3f} {r['dd']:>+7.1f}% {r['pos_rate']:>6.1f}% {marker}")

    best = sorted_results[0] if sorted_results else None
    if best:
        log(f"\n{'='*80}")
        log(f"最优: {best['mode']} 阈值={best['thresh']}, 窗口={best['window']}")
