...
- 截面回归在股票数少时不稳定
- Fold 1（熊市）亏损 -69%，熊市失效更严重

### 结论
❌ 残差动量在 A 股无效，熊市失效更严重。不采纳。

---

## 2026-06-06: v15 质量因子策略验证失败

### 设计
ROE/营收增速/毛利率/负债率 + 残差动量混合。基本面质量因子在 A 股是否有选股能力？
- 数据源：AKShare `stock_financial_abstract_ths`（同花顺财务摘要）
- 4 线程获取全市场 674 只股票，279 秒，0% NaN
- 预计算缓存到 `data/quality_factors_cache.csv`
...
### 结论
❌ 质量因子在 A 股单独用效果差，不如量价因子。不采纳。

---

## 2026-06-06: v16 动量+反转混合策略验证失败

### 设计
短期反转（3-5日）+ 中期动量（20-60日）+ 长期趋势（120日）三周期复合。
...
### 设计
行业动量（20日）+ 行业反转（5日）选 top 5 行业，行业内选股。

### 状态
暂缓。原因：
- 东方财富行业板块接口不稳定（502/连接断开）
- 申万行业分类网站 SSL 证书问题
- 国证行业分类（CNINFO）单只查询太慢（3-5秒/只）
...
### 已创建文件
- `core/industry_rotation.py`：行业轮动因子计算（待数据源就绪后使用）
- `scripts/industry_data_v2.py`：国证行业分类数据获取（多线程）
