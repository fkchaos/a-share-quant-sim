#!/usr/bin/env python3
"""
scripts/backtest/v32_walk_forward.py — v32 分析师预期因子 Walk-Forward 回测
======================================================================
使用 wf_runner 的 run_wf 接口，通过 strategy_adapter 调用选股+风控。

用法:
    python scripts/backtest/v32_walk_forward.py
    python scripts/backtest/v32_walk_forward.py --train 252 --test 63 --step 63
"""
import sys
import os
import time
...
def main():
    parser = argparse.ArgumentParser(description="v32 分析师预期因子 Walk-Forward 回测")
    ...
    print("=" * 60)
    print("v32 分析师预期因子 Walk-Forward 回测")
    print(f"  WF: train={args.train}, test={args.test}, step={args.step}")
    print(f"  区间: {args.start} ~ {args.end}")
    print("=" * 60)

    # ── 运行 WF（通过 wf_runner，使用 strategy_adapter） ──
    results = run_wf(
        strategy_name="v32",
    ...
    if results is None:
        print("\n❌ WF 运行失败（可能是策略未注册到 adapter 或数据不足）")
        return

    # ── 输出结果 ──
    print("\n" + "=" * 60)
    print("v32 Walk-Forward 结果")
    print("=" * 60)
