#!/usr/bin/env python3
"""
scripts/v39c_pv_resonance.py — v39c 多因子评分版
====================================================
目的：验证 v39 的多因子评分逻辑是否比 v27 的简单加分更有效

设计：
- 选股门槛与 v27 完全一致（mom>5%, 排除 pv10<-0.5, 排除退市风险）
- 用 v39 的多因子评分排序替代 v27 的简单加分
- 如果 v39c <= v27，说明多因子评分逻辑无效（证伪）
- 如果 v39c > v27，说明多因子评分逻辑有效
"""
import pandas as pd
import numpy as np
...
DEFAULT_PARAMS = {
    ...
    "MAX_DAILY_BUY": 4,       # 与 v27 一致
    "MAX_POSITION": 0.20,
    "MAX_HOLDINGS": 8,        # 与 v27 一致
    "COOLDOWN_DAYS": 0,
    "MAX_SAME_PREFIX": 0,
...
def select_stocks_v39c(factors, date, current_holdings=None, params=None,
                       sold_recently=None):
    """
    v39c 选股：v27 门槛 + v39 多因子评分

    与 v27 的唯一区别：用多因子评分排序替代简单加分
    """
