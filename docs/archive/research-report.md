...
|------|---------------------|-----------|
| 单股票池策略 | 趋势/多因子/ML 三选一 | 29 因子多策略并行对比 |
| 回测引擎 | 有独立 backtest.py | ✅ `run_backtest.py`（IC分析+Markowitz+参数扫描） |
| 交易约束 | ✅ 完整 | ✅ 完整（同级别） |
| 数据质量 | ✅ 有 | ✅ 有（四维检查） |
| 因子数 | ~11 个横截面因子 | ✅ 29 个技术因子 |
| 因子权重 | 等权或 inverse_vol | ✅ FACTOR_WEIGHTS 可调（29因子独立权重） |
| 评分合成 | Z-score 等权 | ✅ Z-score + 加权/等权/IC-IR/Markowitz |
| 测试覆盖 | 部分 | ✅ 12 个单元测试（smoke + edge） |
| 代码复用 | 回测+模拟盘部分分离 | ✅ core/ 完全共享（零重复） |
| 旧代码清理 | 部分 | ✅ 废弃脚本已归档（archive/） |
| 生产监控 | ✅ 三信号监控 | 📋 待实现 |
