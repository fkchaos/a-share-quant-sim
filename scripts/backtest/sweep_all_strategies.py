#!/usr/bin/env python3
"""
scripts/backtest/sweep_all_strategies.py
全策略重跑 WF，统一条件：2021-06-01 ~ 2026-06-01, 15 folds, zz800
"""
import sys
import os
import re
...
if "{strategy}" in a._risk_params:
    a._risk_params["{strategy}"]["MAX_DAILY_BUY"] = 3
    a._risk_params["{strategy}"]["MAX_POSITION"] = 0.20
...
def main():
    ...
    for i, strat in enumerate(strategies):
        ...
        r = run_one(strat)
        if r:
            mark = "✅" if r['sharpe'] > 0.5 and r['pos_folds'] >= 0.6 * r['total_folds'] else "❌"
            r['name'] = strat
            r['mark'] = mark
            results.append(r)
            ...
            print(f"  FAILED")
            results.append({'name': strat, 'sharpe': 0, 'ret': 0, 'dd': 0, 'pos_folds': 0, 'total_folds': 0, 'mark': '❌'})
