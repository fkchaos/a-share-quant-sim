...
from scripts.strategies.v78_intraday_reversal import calc_factors_v78

def run_ic_analysis():
    """计算IC、IR、分regime IC"""
    print("=" * 60)
    print("v78 日内反转因子 IC分析")
    print("=" * 60)
    ...
    for i in range(30, len(dates) - 5):
        ...
        # 计算因子
        factors = calc_factors_v78(close_slice, vol_slice, amt_slice,
                                   high_slice, low_slice, opn_slice)
        factor_values = factors['v78_intraday_reversal']
    ...
    # 保存结果
    result_file = "/tmp/v78_ic_analysis.txt"
    with open(result_file, 'w') as f:
        f.write("v78 日内反转因子 IC分析结果\n")
        f.write("=" * 40 + "\n")
