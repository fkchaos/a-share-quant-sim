...
| 打分 | 横截面多因子排序，权重可配 |
| 策略 | 可插拔适配器。当前有两个策略在两个独立模拟账户上运行（`v61b`、`v68`） |
...
行情来自公开免费的报价接口，这也是不需要 API key 的原因。代价要说清楚：
**在中国大陆以外，该接口可能很慢、被限频或不可达。** 如果数据接入卡住或返回空表，多半是这个原因。

可替代方案：`akshare`、`baostock`、`tushare`。做一个能把它们随意插拔的适配层已经在 roadmap 上，
也是一个很好的首次贡献。
...
- 可插拔的数据源适配层（`akshare` / `baostock` / `tushare`）
- 更真实的成本与滑点模型
