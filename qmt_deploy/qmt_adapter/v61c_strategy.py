...
def init(C):
    ...
    _hold_days, _last_date = load_hold_days('v61c')
...
def on_signal(C):
    ...
    if _DEBUG and _hold_days:
        for code, days in _hold_days.items():
            print('[%s][V61C] hold: %s days=%d' % (today, code, days))
                        ...
                        if len(df) >= 2:
                            ...
                            if today_close > 0 and prev_close > 0 and today_vol > 0:
                                if c.startswith(('300', '301', '688', '689')):
                                    ...
                                    limit_price = prev_close * 1.295
                                else:
                                    limit_price = prev_close * 1.095
                                if today_close >= limit_price:
                                    if _DEBUG:
                                        print('[V61C] SKIP %s: limit up (%.2f >= %.2f)' % (c, today_close, limit_price))
                                    continue
                    filtered.append(c)
