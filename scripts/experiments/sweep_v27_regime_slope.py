#!/usr/bin/env python3
"""
scripts/backtest/sweep_v27_regime_slope.py — v27 市场状态斜率阈值扫描
=====================================================================
在 strategy_adapter 的 calc_regime 中注入 SLOPE_THRESHOLD 参数，
测试不同斜率阈值对 v27 WF 效果的影响。

扫描空间:
  SLOPE_THRESHOLD: [0.0, 0.0003, 0.0005, 0.0008, 0.001, 0.0015, 0.002]
  REGIME_SLOPE_DAYS: [3, 5, 10]  (计算斜率的时间窗口)
...
print("=" * 70)
print(f"v27 市场状态斜率阈值扫描: {total} 组")
...
for i, combo in enumerate(combos):
    ...
    st = overrides["SLOPE_THRESHOLD"]
    sd = overrides["REGIME_SLOPE_DAYS"]
    label = f"ST={st:.4f} SD={sd}"

    print(f"\n[{i+1}/{total}] {label}")

    # 注入参数到 adapter（regime 参数放 _regime_params，风控参数放 _risk_params）
    for k, v in overrides.items():
        if k.startswith("REGIME_") or k == "SLOPE_THRESHOLD":
...
print(f"\n{'='*70}")
print(f"斜率阈值扫描汇总（按夏普降序）  总耗时: {total_elapsed:.0f}s ({total_elapsed/60:.1f}min)")
print(f"{'='*70}")
print(f"{'参数组合':>30} | {'夏普':>8} | {'收益':>8} | {'回撤':>8} | {'正收益':>10} | {'耗时':>6}")
print("-" * 90)
...
for r in results_sorted:
    ...
    print(f"{r['label']:>30} | {sharpe:>8} | {avg_ret:>7.1f}% | {avg_dd:>7.1f}% | "
